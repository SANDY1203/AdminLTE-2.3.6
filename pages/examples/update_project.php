<?php
    ...
    if ( !empty($_POST)) {
		...
		if (empty($project_name)) {
            $project_nameError = 'Please enter name';
            $valid = false;
        }
        ...
        if ($valid) {
            ...
            $sql = "UPDATE projects set project_name = ?, project_desc = ?, project_company_id = ?, project_price = ?, project_team = ?, project_status = ?, user_id = ?  WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($project_name,$project_desc,$project_company_id,$project_price,$project_team,$project_status,$user_id,$id));
            Database::disconnect();
            if($val == "company")
		{
...
<body>
    <div class="container">
                ...
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="update_project.php?id=<?php echo $id?>&val=<?php echo $val?>" method="post">

                      <div class="control-group <?php echo !empty($project_descError)?'error':'';?>">

                     
					  <div class="control-group <?php echo !empty($project_nameError)?'error':'';?>">

                        <label class="control-label">project_name</label>
                        <div class="controls">
                            <input name="project_name" type="text"  placeholder="project_name" value="<?php echo !empty($project_name)?$project_name:'';?>">
                            <?php if (!empty($project_nameError)): ?>
                                <span class="help-inline"><?php echo $project_nameError;?></span>
                            <?php endif; ?>
                        </div>
                      </div> 
					  <div class="control-group <?php echo !empty($project_descError)?'error':'';?>">
